...
def get_profile(chat_id: str):
    try:
        ...
        if not result.data or len(result.data) == 0:
            return JSONResponse(status_code=404, content={"error": "Профиль не найден"})
        ...
        await httpx.post(
            ...
            json={
                ...
                "reply_markup": {
                    "inline_keyboard": [
                        [
                            {"text": "👀 Посмотреть анкету", "web_app": {"url": f"https://gulyai-webapp.vercel.app/view/{from_chat_id}"}}
                        ],
                        [
